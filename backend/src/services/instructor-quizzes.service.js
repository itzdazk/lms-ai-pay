...
class InstructorQuizzesService extends QuizzesService {
    ...
    async createQuizForLesson({ lessonId, userId, userRole, payload }) {
        ...
        if (existingQuiz) {
            throw this.buildConflictError('Bài học đã có một bài kiểm tra')
        }
        ...
        if (!quizData.title) {
            throw this.buildBadRequestError('Tiêu đề bài kiểm tra là bắt buộc')
        }

        // If questions are provided, validate as array, but not required
        if (payload.questions && !Array.isArray(payload.questions)) {
            throw this.buildBadRequestError('Câu hỏi phải là một mảng')
        }
        ...
        if (
            ...
            const questionsData = this.buildQuestionsDataFromPayload(
                payload.questions,
                created.id
            )
    ...
    async updateQuiz({ quizId, userId, userRole, payload }) {
        ...
        if (Object.keys(updateData).length === 0 && !payload.questions) {
            throw this.buildBadRequestError(
                'Không có cập nhật nào được cung cấp'
            )
        }
    ...
    async setQuizPublishStatus({ quizId, userId, userRole, isPublished }) {
        ...
        if (isPublished) {
            ...
            if (questionCount === 0) {
                throw this.buildBadRequestError(
                    'Bài kiểm tra phải có ít nhất một câu hỏi để được xuất bản'
                )
            }
        }
    ...
    async getQuizAnalytics({ quizId, userId, userRole }) {
        ...
        const passRate =
            totalSubmissions > 0
                ? Number(((passCount / totalSubmissions) * 100).toFixed(2))
                : 0
    ...
    async createQuestion({ quizId, userId, userRole, payload }) {
        ...
        const data = {
            ...
            questionOrder: Number.isInteger(payload.questionOrder)
        ...
        const sanitized = this.sanitizeQuestions([{ ...created }], true)[0]
        return sanitized
    }
    ...
    async updateQuestion({ quizId, questionId, userId, userRole, payload }) {
        ...
        if (!question || question.quizId !== quizId) {
            throw this.buildNotFoundError(
                'Không tìm thấy câu hỏi trong bài trắc nghiệm này'
            )
        }
        ...
        const sanitized = this.sanitizeQuestions([{ ...updated }], true)[0]
        return sanitized
    }
    ...
    async deleteQuestion({ quizId, questionId, userId, userRole }) {
        ...
        if (quiz.isPublished && questionCount === 1) {
            throw this.buildBadRequestError(
                'Không thể xóa câu hỏi cuối cùng khi câu hỏi ôn tập đang ở trạng thái công khai. Vui lòng ẩn trước khi xóa.'
            )
        }
    ...
    async reorderQuestions({ quizId, userId, userRole, orders = [] }) {
        ...
        if (!Array.isArray(orders) || orders.length === 0) {
            throw this.buildBadRequestError(
                'Danh sách thứ tự phải là một mảng và không được để trống.'
            )
        }
        ...
        for (const { questionId } of orders) {
            if (!existingIds.has(questionId)) {
                throw this.buildNotFoundError(
                    `Không tìm thấy câu hỏi ${questionId} trong bài trắc nghiệm này`
                )
            }
        }
