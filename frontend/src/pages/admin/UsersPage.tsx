...
export function UsersPage({ defaultRole }: UsersPageProps = {}) {
    ...
    const handleFilterChange = useCallback(
        (key: keyof GetUsersParams, value: any) => {
            setFilters((prev) => ({
                ...prev,
                [key]: value,
    ...
    const handleChangeRole = (user: User) => {
        setSelectedUser(user)
        const roleLower = (user.role || 'student').toLowerCase() as
            | 'instructor'
            | 'student'
    ...
    const handleChangeStatus = (user: User) => {
        setSelectedUser(user)
        const statusLower = (user.status || 'active').toLowerCase() as
            | 'active'
            | 'inactive'
            | 'banned'
        setNewStatus(
            statusLower.toUpperCase() as 'ACTIVE' | 'INACTIVE' | 'BANNED'
        )
