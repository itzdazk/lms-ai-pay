...
import { Star, Users, BookOpen, Clock, Tag, Bot, Award, GraduationCap, CreditCard } from 'lucide-react';
import { mockCourses, mockCategories, formatPrice, formatDuration } from '../lib/mockData';
  ...
  return (
    <div className="flex flex-col bg-background text-foreground">
      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl mb-4 text-foreground">Tính năng nổi bật</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="bg-[#1A1A1A] border-[#2D2D2D]">
              <CardContent className="pt-6">
                ...
                <h3 className="text-xl font-semibold mb-2 text-white">AI Tutor Chatbox</h3>
                <p className="text-gray-400">Chat trực tiếp với AI để được hỗ trợ học tập 24/7</p>
              </CardContent>
            </Card>
            <Card className="bg-[#1A1A1A] border-[#2D2D2D]">
              <CardContent className="pt-6">
                <Award className="h-12 w-12 text-green-500 mb-4" />
                <h3 className="text-xl font-semibold mb-2 text-white">Auto-grading Quiz</h3>
                <p className="text-gray-400">Tự động chấm điểm và hiển thị kết quả ngay lập tức</p>
              </CardContent>
            </Card>
            <Card className="bg-[#1A1A1A] border-[#2D2D2D]">
              <CardContent className="pt-6">
                ...
                <p className="text-gray-400">Theo dõi tiến độ học tập chi tiết và tự động</p>
              </CardContent>
            </Card>
            <Card className="bg-[#1A1A1A] border-[#2D2D2D]">
              <CardContent className="pt-6">
                <CreditCard className="h-12 w-12 text-orange-500 mb-4" />
                <h3 className="text-xl font-semibold mb-2 text-white">Payment Integration</h3>
                <p className="text-gray-400">Thanh toán dễ dàng với VNPay và MoMo</p>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-12">
            <div>
              <h2 className="text-3xl md:text-4xl mb-4 text-foreground">Khóa học nổi bật</h2>
                ...
                <Card key={course.id} className="overflow-hidden hover:shadow-lg transition-shadow flex flex-col bg-[#1A1A1A] border-[#2D2D2D]">
                  ...
                  <CardContent>
                    ...
                    <div className="flex items-center justify-between w-full">
                      {course.is_free ? (
                        <span className="text-2xl text-green-500">Miễn phí</span>
                      ) : (
                        <div>
      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl mb-4 text-foreground">Khám phá theo danh mục</h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {mockCategories.map(category => (
              <Link
                ...
                className="group"
              >
                <Card className="bg-[#1A1A1A] border-[#2D2D2D] hover:border-primary transition-colors cursor-pointer h-full">
