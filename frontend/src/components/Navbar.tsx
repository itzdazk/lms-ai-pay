    ...
    <header className="sticky top-0 z-50 w-full border-b border-[#2D2D2D] bg-black">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="hidden md:flex flex-1 max-w-xl mx-8">
          <div className="relative w-full">
            ...
            <Button
              size="icon"
              variant="ghost"
              className={`absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 ${isListening ? 'text-red-500 animate-pulse' : 'text-gray-400 hover:text-white'}`}
        ...
        <nav className="hidden lg:flex items-center gap-6">
          <Link to="/courses" className="text-gray-300 hover:text-blue-600 transition-colors">
            Khóa học
          </Link>
          <Link to="/ai-chat" className="text-gray-300 hover:text-blue-600 transition-colors">
            AI Chat
          </Link>
        ...
        <div className="flex items-center gap-3">
            ...
            <DropdownMenuContent align="end" className="w-[480px] max-w-[90vw] bg-[#1A1A1A] border-[#2D2D2D]">
              ...
              <div className="max-h-[420px] overflow-y-auto divide-y divide-[#2D2D2D]">
                <div className="p-4 hover:bg-[#1F1F1F] cursor-pointer">
                  <p className="text-sm text-white font-semibold">Bạn đã hoàn thành bài học "React Hooks"</p>
                  <p className="text-xs text-gray-500 mt-1">2 giờ trước</p>
                </div>
                <div className="p-4 hover:bg-[#1F1F1F] cursor-pointer">
                  <p className="text-sm text-white font-semibold">Khóa học "Next.js Pro" vừa được cập nhật</p>
                  <p className="text-xs text-gray-500 mt-1">Hôm qua</p>
                </div>
                <div className="p-4 hover:bg-[#1F1F1F] cursor-pointer">
                  <p className="text-sm text-white font-semibold">Bạn có chứng chỉ mới cần tải xuống</p>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-[#1A1A1A] border-[#2D2D2D]">
              ...
              <DropdownMenuSeparator className="bg-[#2D2D2D]" />
              <DropdownMenuItem className="text-red-400 hover:bg-[#1F1F1F] cursor-pointer">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="lg:hidden text-white hover:bg-[#1F1F1F]">
                <Menu className="h-5 w-5" />
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="w-[300px] bg-[#1A1A1A] border-[#2D2D2D]">
              <SheetHeader>
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="relative">
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    className={`absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 ${isListening ? 'text-red-500' : 'text-gray-400'}`}
                    onClick={handleVoiceSearch}
                  >
                    <Mic className="h-4 w-4" />
                  </Button>
                ...
                <div className="pt-4 border-t border-[#2D2D2D]">
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <button
                      className="block w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-[#1F1F1F] rounded-lg transition-colors"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      Đăng xuất
                    </button>
